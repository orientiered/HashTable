...
В результате время 100 млн поисков удалось уменьшить с 16.7 +- 0.1 секунд до 1.850+-0.006 секунд. (Ускорение в 9.0+-0.1 раз).

## Содержание

1. [Теория](#немного-теории)
2. [Постановка задачи](#что-оптимизируем)
3. [Компиляция](#компиляция)
4. [Для программистов](#для-программистов)
5. [Анализ хеш-функций](#хеш-функции)
6. [Методика тестирования](#методика-тестирования)
    + [Тестовые данные](#тестовые-файлы)
    + [Измерение времени](#измерение-времени)
    + [Погрешности](#о-погрешностях)
    + [Троттлинг](#троттлинг)
    + [О load factor](#важно-load-factor)
7. [Ход работы](#ход-работы-оптимизации)
    + [Анализ первой версии](#тесты-первой-версии)
    + [Оптимизация strcmp](#оптимизация-strcmp)
    + [Оптимизация функции поиска](#оптимизация-bucketsearch)
    + [Оптимизация хеш-функции](#оптимизация-хеш-функции-crc32)
    + [Правильный load factor](#тесты-с-правильным-load-factor)
8. [Вывод](#вывод)
...
1. **Метод цепочек**
    ...
    <figure>
        <img src="docs/hashTable_list.svg">
        <figcaption>Рис. 1: Поиск слова в хеш-таблице на списках </figcaption>
    </figure>

    На рисунке 1 показан пример поиска слова в таблице. В одном списке лежат элементы, у которых совпал хеш по модулю размера массива. Значения не показаны для простоты.

2. **Открытая адресация**

    В этом случае хеш-таблица - **циклический массив** пар ключ-значение. В этом массиве лежат все элементы, добавленные в таблицу. Аналогично предыдущему способу для данного ключа находится индекс в этом массиве, а затем начинается **пробирование** - поиск по определённому алгоритму нужного элемента в массиве/места для вставки нового. В случае вставки элемент ставится на первое найденное свободное место.
    ...
    <figure>
        <img src="docs/hashTable_open_addr.svg">
        <figcaption>Рис. 2: Визуализация квадратичного пробирования</figcaption>
    </figure>


    На рисунке 2 показано, как мог выглядеть массив и поиск слова "Hello" в нём. Если у разных ключей совпал индекс в массиве, то они будут лежать на одном пути пробирования. При этом эти цепочки пробирования также могут пересекаться.

    На рисунке, в качестве примера, в ячейку с индексом `k+1` попал элемент "Elephant" (выделен чёрным), у которого индекс старта пробирования - `k`, хотя мы ожидали найти там одно из слов, написанных синим. Однако дальше коллизий не было, поскольку шаг поиска меняется.

    Если бы мы использовали линейное пробирование, то в путях поиска на примере совпали бы все ячейки, кроме первой, т.е. коллизий было бы гораздо больше.

    Главный плюс открытой адресации - хорошая локальность, благодаря которой данные лучше ложатся на кеш процессора. Из минусов - он требует больше памяти, так как заполнение массива более чем на 75% (характерное значение) сильно увеличивает число коллизий, тем самым замедляя работу программы.

**В данной работе используется метод цепочек для разрешения коллизий, поскольку он проще в реализации.**
...
`make ASAN=1` - собрать в дебажной версии с санитайзером

`make BUILD=PERF` - собрать в релизной версии с флагами для профилирования

`make BUILD=RELEASE` - собрать в релизной версии

`make clean` - очистить результат предыдущей сборки

`make test_file` - создать тестовые файлы

Доступные опции:
+ `TESTS=10000000` - кол-во строк в тестовом файле
+ `FOUND_PERCENT=0.9` - процент строк, которые будут взяты из файла (остальные случайные)
+ `TEST_FILE=shakespeare.txt` - файл, из которого брать строки

`make perfTest` - провести профилирование программы, предварительно скомпилировав её в режиме `BUILD=PERF`

`make perfStat` - собрать статистику аппаратных счётчиков производительности (процент промахов кеша и т.п.)

`make compile_commands` - создать файл `compile_commands.json` для работы с clangd

`make run` - пересобрать в релизной версии и запустить программу 6 раз с прикреплением к одному ядру

`make dump` - дизассемблировать программу в файл `dump.s`

+ Основные настройки программы находятся в файле [hashTable.h](include/hashTable.h).

+ Размер хеш-таблицы и количество циклов тестирования меняется в файле [perfTester.h](include/perfTester.h)
...
Для эффективной работы таблицы элементы должны быть равномерно распределены по бакетам. Для оценки распределения есть функция `hashTableCalcDistribution`, которая считает дисперсию количества элементов в бакетах и строит в консоли график распределения размера бакета (кол-во коллизий) по номеру бакета.
...
Посмотрим на гистограмму распределения колличества колизий по количеству бакетов с таким размером:

| Checksum | Djb2 |
|----------|------|
|![checksum hist](docs/checksum_dist.svg) | ![djb2_hist](docs/djb2_dist.svg) |

![crc32_hist](docs/crc32_dist.svg)

Если хеш-функция действительно случайна (распределение не зависит от номера бакета) (здесь за случайную величину принимаем индикатор того, что элемент попал в бакет с фиксированным номером), то, согласно центральной предельной теореме, сумма большого числа таких событий (= размер бакета) будет стремиться к нормальному распределению.

Гистограмма `checksum` имеет максимум в 0 - большинство бакетов оказались пустыми, `djb2` выглядит немного похожим на Гауссово распределение, но всё ещё далёк от него.

Гистограмма `crc32` довольна близка к нормальному распределению, причём её максимум (17) совпадает с мат. ожиданием (16.4). Её дисперсия известна, поэтому можно сделать оценку: 67% бакетов имеют длину от 13 до 21 элементов.

## Методика тестирования

Машина: Lenovo XiaoXin X16 Pro (2024)

CPU: AMD Ryzen 7 8845H w/ Radeon 780M Graphics 3.80 GHz, boost up to 5.14GHz (8 cores, 16 threads)
...
Поскольку оптимизируется функция поиска, то наибольший интерес представляет время, проведенное в цикле подсчёта слов из файла 2.
Время, затраченное на подготовку данных и загрузку слов из файла 1 в таблицу, не учитывается, так как это уже совсем другая задача.
...
Первый запуск проводится без флага `-s`, который отключает весь вывод программы, кроме времени, затраченного на поиск.

Частота процессора не фиксировалась, но ограничение на неё было снято при помощи команды `sudo cpupower frequency-set --max 5.14 GHz). Так можно получить время, которое лучше соотносится с тем, что будет при реальном использовании программы.

Все тесты проведены, когда ноутбук подключен к электрической сети (на зарядке).

Во время тестов все программы закрыты, кроме текстового редактора VSCode.

Серия тестов проводится несколько раз, чтобы убедиться в воспроизводимости результатов.

### Троттлинг

Его нет. Тест достаточно короткий (около 10 секунд), поэтому процессор не успевает нагреться выше 65 градусов. (температура измерялась при помощи утилит `psensor` и `btop`)
...
|---------------|-----------------|----------|--------|-------------------------|
| -O3           | (пред.версия)   | 14.5  +- 0.1  | 550 +- 4 | 1                |
| Да            | Нет             | 12.17 +- 0.06 | 462 +- 2 | 1.19 +- 0.01     |
| Нет           | Да              | 12.93 +- 0.04 | 490 +- 2 | 1.12 +- 0.01     |
| Да            | Да              | 12.14 +- 0.05 | 461 +- 2 | 1.19 +- 0.01     |

Время работы уменьшилось в 1.2 раза. При этом предварительное сравнение длин строк даёт улучшение в пределах погрешности при совместном использовании с `FAST_STRCMP`.

Проведём профилирование, отключив предварительное сравнение длин строк:

![strcmp_opt](docs/hotspot_1.png)

Теперь сравнение строк (`fastStrcmp` - его части выделены **красным**) занимает порядка **32%** - заметное улучшение. Стоит отметить, что появились **накладные расходы** на копирование ключа и поиск его длины. На рисунке они выделены **синим** и занимают около **7%**.
...
|---------------|-----------------|----------|-------|
| `FAST_STRCMP` | 12.17 +- 0.06 | 462 +- 2 | 0% |
| Загрузка маскированием | 12.11 +- 0.04 | 459  +- 1.3 | 0.6% +- 0.7% |
...
| Arch 2              | 3.20 +- 0.02  | 121.3 +- 0.7 | 3.87 +- 0.04 |
| Arch 2 + FastStrcmp | 2.43 +- 0.01  | 92.3 +- 0.5  | 5.09 +- 0.05 |
...
```
    ...
    Arch 2 version (with FAST_STRCMP)
...
Выровненные ключи (5) позволяют загрузить их в SIMD регистр одной инструкцией (соответствующий интринсик `_mm_load_si128`), без маскирования.

| Версия | Время,с | Такты | Относительное ускорение |
|------|--------|--------|--------------------|
| Arch 2 + FAST_STRCMP  | 2.43 +- 0.01  | 92 +- 0.5 | 1|
| Arch 2 + aligned keys |  2.27 +- 0.01 | 86.1 +- 0.5 | 1.07 +-0.01 |
...
Теперь попробуем хранить короткие данные (до 16 байт) прямо в структуре ноды (архитектурная оптимизация 4). В теории это улучшит локальность и уменьшит количество доступов к памяти. Проведём тестирование:

| Версия | Время,с | Такты | Относительное ускорение |
|------|--------|--------|--------------------|
| Arch 2 + aligned keys |  2.27 +- 0.01 | 86.1 +- 0.5 | 1 |
| Arch 2 + `SHORT_VALUES_IN_NODE`  | 2.21 +- 0.01  | 83.9 +- 0.4 | 1.026 +- 0.01 |

Программа ускорилась, но прирост довольно мал.

Посмотрим ещё раз на статистику промахов кеша:

```
     1 891 906 895      cache-references
        55 561 472      cache-misses                     #    2,94% of all cache refs
    13 560 129 141      cycles
    21 702 218 946      instructions                     #    1,60  insn per cycle
     4 545 092 466      branches
            79 402      faults
                 0      migrations
```

Процент промахов упал с 5% до 3%.

### Оптимизация хеш-функции CRC32

Проведём профилирование со всеми сделанными оптимизациями:
...
|--------|----------|-------|------|
| Arch2 + ... | 2.21 +- 0.01 | 83.9 +- 0.4 | 1 |
| += fastCrc32u | 2.05 +- 0.01 | 77.7 +- 0.4 | 1.09 +- 0.01 |
...
**Примечание**: несмотря на то, что внутри функции есть ассемблер, современный `gcc` инлайнит её.

Результаты выполнения с ней:

|Hash| Время, с | Такты | Относительное улучшение |
|----|----------|-------|-------------------------|
| fastCrc32u | 2.05 +- 0.01 | 77.7 +- 0.4 | 1 |
|fastCrc32_16| 1.850 +- 0.006 | 70.2 +- 0.2 | 1.11 +-0.01 |

Прирост более чем на 10% по сравнению с предыдущим вариантом.

### Тесты с правильным load factor

| Версия | Время, с | Такты | Относительное улучшение |
|----|----------|-------|-------------------------|
|fastCrc32_16| 1.850 +- 0.006 | 70.2 +- 0.2 | 1 |
| LoadFactor=1.64 | 1.28 +- 0.01 | 48.4 +- 0.3 | 1.45 +- 0.01 |
| L.F. = 1.64, no asm/intrin | 2.23 +- 0.02 | 84.6 +- 0.8 | - |
| L.F. = 1.64, arch 1, no asm/intrin | 4.05 +- 0.04 | 154 +- 2 | - |

Поясним последние две строчки таблицы: эти замеры нужны, чтобы понять, как быстро могла бы работать программа без аппаратно-зависимых оптимизаций. Они соответсвуют второй и первой архитектурам программы соответственно.
...
Следующим шагом оптимизаций может быть оптимизирование strlen: длины всех строк известны на этапе подготовке данных и их можно хранить. Сейчас поиск длины строки занимает порядка 15% времени.

Также можно попытаться переписать на ассемблере функцию поиска и/или сделать развёртку цикла в `bucketSearch`.

| Оптимизация            | Время, с       | Такты        | Относительное улучшение |
|------------------------|----------------|--------------|-------------------------|
| Odefault               | 16.7  +- 0.1   | 634 +- 4     | 1                       |
| -O3                    | 14.5  +- 0.1   | 550 +- 4     | 1.15                    |
| `FAST_STRCMP`          | 12.17 +- 0.06  | 462 +- 2     | 1.37                    |
| Arch 2                 | 3.20 +- 0.02   | 121.3 +- 0.7 | 5.23                    |
| Arch 2 + FastStrcmp    | 2.43 +- 0.01   | 92.3 +- 0.5  | 6.87                    |
| aligned keys           | 2.27 +- 0.01   | 86.1 +- 0.5  | 7.36                    |
| `SHORT_VALUES_IN_NODE` | 2.21 +- 0.01   | 83.9 +- 0.4  | 7.56                    |
| fastCrc32u             | 2.05 +- 0.01   | 77.7 +- 0.4  | 8.16                    |
| fastCrc32_16           | 1.850 +- 0.006 | 70.2 +- 0.2  | 9.03 +- 0.08            |
...
Выравнивание ключей дало ощутимый прирост производительности, так как это позволило хешировать по 8 симоволов строки за раз.

Проанализируем ускорение от оптимизаций, которые требовали использование ассемблера или интринсиков (аппаратно-зависимый код).

| Оптимизация           | Время, с       | Такты       | Улучшение отн. предыдущего |
|-----------------------|----------------|-------------|----------------------------|
| -O3                   | 14.5  +- 0.1   | 550 +- 4    | 1                          |
| Arch 2, no asm/intrin | 3.12 +- 0.01   | 118 +- 0.5  | 4.66                       |
| Arch 2, all opt       | 1.850 +- 0.006 | 70.2 +- 0.2 | 1.68                       |
| LoadFactor=1.64       | 1.28 +- 0.01   | 48.4 +- 0.3 | 1.45                       |

Они дают ускорение на 70%.

**Примечание**: после последней оптимизации (fastCrc32_16) время подготовки файлов и загрузки строк в таблицу занимает примерно 30% от общего времени, поэтому предположение о том, оптимизация поиска была приоритетнее - оправдано.
